...
public class DTNSimGUI extends DTNSimUI {
	...
	private void initGUI() {
		...
		this.main = new MainWindow(this.scen.getName(), world, field, guiControls, infoPanel, eventLogPanel, this);
		...
		this.main.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				if (closeAgain) {
					...
					System.err.println("Forced close. " + "Some reports may have not been finalized.");
					System.exit(-1);
				}
				closeAgain = true;
			}
	...
	private void processAssertionError(AssertionError e) {
		...
		String txt = (msg != null ? msg : "") + " at simtime " + SimClock.getIntTime() + "\n\ncaught at:\n"
		...
		int selection = JOptionPane.showOptionDialog(getParentFrame(), txt, title, JOptionPane.YES_NO_OPTION,
	...
	public void update(boolean forcedUpdate) {
		...
		if (!forcedUpdate && guiUpdateInterval > (SimClock.getTime() - this.lastUpdate)) {
			return;
		}
	...
	public Coord getCenterViewCoord() {
		...
		midX = sp.getHorizontalScrollBar().getValue() + sp.getViewport().getWidth() / 2;
		midY = sp.getVerticalScrollBar().getValue() + sp.getViewport().getHeight() / 2;
	 ...
	 */
	private class PlayfieldMouseHandler extends MouseAdapter implements MouseWheelListener {
		/**
